...
class QueueingSerialServer(serial_server.SerialServer):
    ...
    def __init__(self, device: str,
        ...
        # We don't need to poll the queue
        def on_new_entries() -> None:
            asyncio.ensure_future(self._process_queue())

        self._queue = asyncio_tools.DeDupQueue(on_enqueue=on_new_entries)
    ...
    async def async_serve(self):
        ...
        await asyncio.get_event_loop().run_in_executor(None, self.serve)
    ...
    async def _process_queue(self) -> None:
        ...
        # Due to the async nature of this class the queue might well be empty
        # at the time this starts executing, even though it's a callback on a
        # freshly filled queue (see ``__init__``).
        if not n_items:
            return
        try:
            while True:
